...
func TestTransactionLoading(t *testing.T) {
	table := []struct {
	...
	}{
		{
			...
			processed: true,
			resultRows: []string{
				"1, 1, sell, AMZN , 1 , 5000 , 2020-01-01 , false, 1",
			},
			resultTransaction: []Transaction{
				Transaction{
					UserID:         1,
		...
		},
		{
			...
			resultRows: []string{
				"1, 1, sell, AMZN , 1 , 5000 , 2020-01-01 , false, 1",
				"1, 1, sell, AMZN , 1 , 5000 , 2020-01-01 , true ,1",
				"2, 2, buy, AMZN , 1 , 5000 , 2020-01-01 , false ,1",
			},
			resultTransaction: []Transaction{
				Transaction{
					UserID:         1,
	...
	for index := range table {
		columns := []string{"id", "userid", "action", "symbol", "amount", "value", "date", "processed", "current_balance"}
		entry := table[index]
...
func TestTransactionWriting(t *testing.T) {
	...
	for index := range table {
		...
		result := entry.transaction.Write(entry.transaction.Processed, handler, "1")
		handler.Commit()
		if err = mock.ExpectationsWereMet(); err != nil {
			t.Error(err)
		} else {
